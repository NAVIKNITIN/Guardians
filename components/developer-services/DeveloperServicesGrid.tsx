...
const SERVICE_TILES: ServiceTile[] = [
  ...
  },
  {
    ...
    imageSrc:
      "/images/Buyer/services/marketing.svg",
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-5 px-12 py-[17px]">
        <span className="n-reg   uppercase tracking-[0.1em] text-white">
          {tile.label}
        </span>
        <ChevronRight />
      </div>
...
// ─── Commercial Services accordion panel ──────────────────────────────────────
  ...
  return (
    <div className="relative h-full min-h-[520px] overflow-hidden bg-[#D5D3D4] lg:min-h-[560px]">
      ...
      <div className="relative z-10 flex h-full flex-col px-8 py-12 lg:px-10">
        <h2 className="mb-8 qs-reg xt-[clamp(2rem,4vw,3.125rem)]  uppercase tracking-[0.05em] text-white">
          Commercial Services
        </h2>
        ...
        <div className="flex flex-1 flex-col">
          {ACCORDION_ITEMS.map((item, i) => (
            <div key={item.title}>
              ...
              >
                <span className="n-reg xl  leading-snug text-white">
                  {item.title}
                </span>
                <span className="shrink-0 n-reg  text-xl  text-white">
                  {openIndex === i ? "−" : "+"}
                </span>
              </button>
              {openIndex === i && item.description && (
                <p className="pb-4 n-reg  text-base  leading-[1.5] text-white">
                  {item.description}
                </p>
              )}
            </div>
        ...
        <div className="mt-auto pt-8">
          <Link
            href="#"
            className="inline-flex items-center gap-5 n-reg  text-2xl  uppercase tracking-[0.1em] text-white transition-opacity hover:opacity-80"
          >
            Know More
            <ChevronRight />
          </Link>
...
export function DeveloperServicesGrid() {
  return (
    <section className="bg-white mb-2 lg:mb-[80px]" aria-label="Developer services">
